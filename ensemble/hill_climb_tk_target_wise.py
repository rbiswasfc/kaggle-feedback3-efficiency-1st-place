...
def main():
    ...
    # Added by TK to have common oof dfs list and zero wts for unused
    print(OOF)
    fin_wts = np.zeros(len(OOF))
    for i, idx in enumerate(m):
        fin_wts[idx] = final_wts[i]
    print("[")
    print(','.join([str(x) for x in fin_wts]))
    print("],")
