...
def rmse(y_pred: np.ndarray, y_true: np.ndarray) -> float:
    assert y_pred.shape == y_true.shape, "y_true and y_pred shape must be the same"
    return float(np.linalg.norm(y_pred - y_true) / np.sqrt(y_pred.shape[0]))

def mcrmse(y_pred: np.ndarray, y_true: np.ndarray) -> float:
    assert len(y_pred.shape) == len(y_true.shape) == 2, "y_true and y_pred must be 2d"
    assert y_pred.shape[0] == y_true.shape[0], "y_true and y_pred must have the same number of columns"
    return np.mean([rmse(y_pred[i], y_true[i]) for i in range(y_pred.shape[0])])
