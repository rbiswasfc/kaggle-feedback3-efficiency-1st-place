...
try:
    from kw_prefix.fb_dataloader import CustomDataCollatorWithPadding
    from kw_prefix.fb_dataset import FeedbackDataset
    from kw_prefix.fb_model import AWP, FeedbackModel
...
@hydra.main(version_base=None, config_path="../conf/kw_prefix", config_name="conf_kw_prefix")
def run_training(cfg):
    ...
    all_ids = df["text_id"].unique().tolist()
    ...
    if cfg.ensemble_pl.use_pl:
        raw_df = pd.read_csv(os.path.join(cfg.ensemble_pl.pl_folder, "pl_data.csv"))
        raw_df["full_text"] = raw_df["full_text"].apply(lambda x: x.replace("[LF]", "\n\n"))

        if cfg.all_data:
            dfs = []
            for fold_id in range(8):
                pl_labels_path = os.path.join(cfg.ensemble_pl.pl_labels_folder, f"fold{fold_id}_pl.csv")
                print(f"loading PL labels from {pl_labels_path}")
                print(f"loading pl labels from: {pl_labels_path}")
                tmp = pd.read_csv(pl_labels_path)
                dfs.append(tmp)
            labels_df = pd.concat(dfs, axis=0)
            print(labels_df.head(10))
            labels_df = labels_df.groupby("text_id")["cohesion", "syntax", "vocabulary", "phraseology", "grammar", "conventions"].agg(np.mean).reset_index()
            print(labels_df.head())
        else:
            pl_labels_path = os.path.join(cfg.ensemble_pl.pl_labels_folder, f"fold{cfg.fold}_pl.csv")
            print(f"loading PL labels from {pl_labels_path}")
            print(f"loading pl labels from: {pl_labels_path}")
            labels_df = pd.read_csv(pl_labels_path)
        pseudo_df = pd.merge(raw_df, labels_df, on='text_id', how='left')

        # remove valid samples + sample
        pseudo_df = pseudo_df[~pseudo_df["text_id"].isin(all_ids)].copy()
        pseudo_df = pseudo_df.reset_index(drop=True)
        pseudo_df["original_text_id"] = pseudo_df["text_id"].apply(lambda x: x.split("_")[0])
        ...
        if cfg.all_data:
            ...
            pseudo_df = pseudo_df[~pseudo_df["original_text_id"].isin(valid_ids)].copy()
        print(f"shape of pseudo df after removing valid ids: {pseudo_df.shape}")

        # sampling
        selected_ids = set()
        pl_samples_dir = cfg.ensemble_pl.pl_samples_dir
        
        for fold_id in range(4):
            with open(os.path.join(pl_samples_dir, f"pl_ids_{fold_id}.json"), "r") as f:
                these_selected_ids = json.load(f)
            selected_ids.update(these_selected_ids)
        
        print(f"# of PL samples: {len(selected_ids)}")
        pseudo_df = pseudo_df[pseudo_df["text_id"].isin(selected_ids)].copy()
        pseudo_df = pseudo_df.drop(columns=["original_text_id"])
        pseudo_df = pseudo_df.reset_index(drop=True)
        print(f"shape of unlabelled dataset for pseudo labelling {pseudo_df.shape}")
    ...
    train_ds.set_format(
        type=None,
        columns=['input_ids', 'attention_mask', 'token_type_ids', 'sequence_ids', 'labels']
    )
    ...
    if cfg.train_params.use_ema:
        print_line()
        decay_rate = 0.99  # torch.exp(torch.log(cfg.train_params.ema_prev_epoch_weight) / num_update_steps_per_epoch)
