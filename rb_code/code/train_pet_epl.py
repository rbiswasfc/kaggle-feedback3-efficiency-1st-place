...
try:
    from pet.fb_dataloader import CustomDataCollatorWithPadding
    from pet.fb_dataset import FeedbackDataset
    from pet.fb_model import AWP, FeedbackModel, FeedbackModelAdaPET
...
@hydra.main(version_base=None, config_path="../conf/pet", config_name="conf_pet")
def run_training(cfg):
    ...
    if cfg.t5_dataset.use_t5:
        ...
        df_t5["full_text"] = df_t5["full_text"].apply(lambda x: x.replace("[LF]", "\n\n"))
    ...
    if cfg.ensemble_pl.use_pl:
        ...
        if cfg.all_data:
            ...
            for fold_id in range(4):
                # ens001_fold0
                pl_labels_path = os.path.join(cfg.ensemble_pl.pl_labels_folder, f"ens001_fold{fold_id}.csv")
            ...
            pl_labels_path = os.path.join(cfg.ensemble_pl.pl_labels_folder, f"ens001_fold{cfg.fold}.csv")
            print(f"loading PL labels from {pl_labels_path}")
            print(f"loading pl labels from: {pl_labels_path}")
    ...
    train_ds.set_format(
        type=None,
        columns=['input_ids', 'attention_mask', 'token_type_ids', 'mask_token_idxs', 'labels']
    )
    ...
    if cfg_dict["model"]["use_adapet"]:
        print("using adapet model...")
        model = FeedbackModelAdaPET(cfg_dict)
    else:
        print("using pet model...")
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(train_dl):
            batch["pos_tok_ids"] = pos_tok_id_list
            batch["neg_tok_ids"] = neg_tok_id_list
