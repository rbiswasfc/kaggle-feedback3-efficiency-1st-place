...
try:
    from multiscale.fb_dataloader import CustomDataCollatorWithPadding
    from multiscale.fb_dataset import FeedbackDataset
    from multiscale.fb_model import AWP, FeedbackModel
    from utils.optim_utils import get_optimizer
    from utils.train_utils import (EMA, AverageMeter, get_lr, get_score,
...
@hydra.main(version_base=None, config_path="../conf/multiscale", config_name="conf_multiscale")
def run_training(cfg):
    ...
    train_ds.set_format(
        ...
        columns=['input_ids', 'attention_mask', 'token_type_ids', 'span_head_idxs',
                 'sentence_tail_idxs', 'sentence_head_idxs', 'span_tail_idxs', 'labels']
    )
    ...
    for epoch in range(num_epochs):
        ...
        sim_loss_meter = AverageMeter()
        rank_loss_meter = AverageMeter()
        bt_loss_meter = AverageMeter()
        ...
        for step, batch in enumerate(train_dl):
            ...
            if (step + 1) % grad_accumulation_steps == 0:
                ...
                try:
                    sim_loss_meter.update(loss_dict["sim_loss"].item())
                    rank_loss_meter.update(loss_dict["ranking_loss"].item())
                    bt_loss_meter.update(loss_dict["bt_loss"].item())
                ...
                if cfg.use_wandb:
                    ...
                    wandb.log({"sim_loss": round(sim_loss_meter.avg, 5)}, step=wandb_step)
                    wandb.log({"rank_loss": round(rank_loss_meter.avg, 5)}, step=wandb_step)
                    wandb.log({"bt_loss": round(bt_loss_meter.avg, 5)}, step=wandb_step)
