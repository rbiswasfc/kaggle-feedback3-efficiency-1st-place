...
try:
    from efficiency.fb_dataloader import CustomDataCollatorWithPadding
    from efficiency.fb_dataset import FeedbackDataset
    from efficiency.fb_model import AWP, FeedbackModel
...
@hydra.main(version_base=None, config_path="../conf/efficiency", config_name="conf_efficiency")
def run_training(cfg):
    ...
    if cfg.ensemble_pl.use_pl:
        ...
        # # ------
        # if cfg.all_data:
        #     pl_samples_dir = cfg.ensemble_pl.pl_essay_dir
        #     dfs = []
        #     for fold_id in range(cfg.fold_metadata.n_folds):
        #         tmp = pd.read_csv(os.path.join(pl_samples_dir, f"fold{fold_id}_pl.csv"))
        #         print(tmp.head())
        #         dfs.append(tmp)

        #     for fold_id in range(8):
        #         tmp = pd.read_csv(os.path.join("../datasets/fb3-en-pl-8f/8f_4m", f"fold{fold_id}_pl.csv"))
        #         print(tmp.head())
        #         dfs.append(tmp)

        #     pseudo_df = pd.concat(dfs, axis=0)
        #     print(pseudo_df.head(10))
        #     pseudo_df = pseudo_df.groupby("text_id")["cohesion", "syntax", "vocabulary", "phraseology", "grammar", "conventions"].agg(np.mean).reset_index()
        #     print(pseudo_df.head())
        # else:
        #     pseudo_df = pd.read_csv(cfg.ensemble_pl.pl_essay_path)

        # pseudo_df = pd.merge(pseudo_df, raw_df, on="text_id", how="inner")
        pseudo_df = pd.read_csv("../datasets/processed/pl_ens023.csv")

        pseudo_df["full_text"] = pseudo_df["full_text"].apply(
            lambda x: x.replace("[LF]", "\n\n")
        )
        ...
        # else:
        #     with open(cfg.ensemble_pl.pl_samples_path, "r") as f:
    ...
    if cfg.train_params.use_ema:
        print_line()
        decay_rate = 0.992  # torch.exp(torch.log(cfg.train_params.ema_prev_epoch_weight) / num_update_steps_per_epoch)
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(train_dl):
            ...
            if (step + 1) % grad_accumulation_steps == 0:
                ...
                try:
                    contrastive_loss_meter.update(loss_dict["contrastive_loss"].item())
                except Exception as e:
                    pass
                ...
                if cfg.use_wandb:
                    ...
                    wandb.log({"contrastive_loss": round(contrastive_loss_meter.avg, 5)}, step=wandb_step)
